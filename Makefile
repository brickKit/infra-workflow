IMAGE   := brickenterprise/infra-workflow
...
test:  ## 需要 TEST_PG_DSN，缺省时连库测试自动跳过
...
migrate-idempotent:  ## 同一份迁移连跑两次都必须成功（§13.3 铁律五）
	...
	@# infra_workflow_rw）：建分区之类的 DDL 需要建表权限。
	@go build -o /tmp/infra-workflow-migrate-probe ./backend/cmd/migrate
	@/tmp/infra-workflow-migrate-probe up && /tmp/infra-workflow-migrate-probe up && echo "✓ 迁移幂等"

dag-check:  ## 强依赖图无环（§4.2）。零强依赖零弱依赖，天然无环——本组件在同步图上只有入边（§6.6 铁律三：严禁反向同步调用业务组件）
	@if ! grep -qE '^\s*components:\s*\[\]' component.yaml; then \
	   echo "✗ infra-workflow 依赖必须为空（§6.6 铁律三：本组件在同步图上只有入边）"; exit 1; fi
	@echo "✓ 零依赖，无环"
...
import-scan:  ## 铁律六：不许 import 任何其他组件仓库（§13.3）
	@bad="$$(go list -deps ./... 2>/dev/null | grep -E '^github.com/brickKit/' \
	         | grep -vE '^github.com/brickKit/(infra-workflow|be-sdk-go)(/|$$)' || true)"; \
	 if [ -n "$$bad" ]; then \
...
docs-check:  ## 四份文档结构检查（总纲 §4 SOP-D）
	@bash ../../../infra/scripts/docs-check.sh infra-workflow

smoke:  ## 原则一：只装这一个组件就能起来（§1.5、§3.11 第 8 条）
	@# brickkit 不向上找 brickkit.yaml，必须从装配仓库根目录跑——本组件
	@# 固定挂在 components/infra/workflow 下，根目录固定是 ../../..
